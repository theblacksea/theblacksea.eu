...
function Bubble(data) {
	...
	d3.select("#bubbles > svg > defs")
				.append("svg:circle")
					.attr("id", "country-count-bg").attr("r", 24)
...
//					.attr("stroke", 'red')
	...
	var nodes = [], treemap;
	var period ={start: new Date('Jan 01, 2014'), end: new Date('Sep 30, 2015')};
	...
	var statusRadius = {	"Operational": 25,
							"Taken over": 30,
							"Closed": 20,
							"Reopened": 25,
							"Liquidated": 20,
							...
							"Destroyed": 15,
							"Moved to Kyiv": 27,
							"Moved to Zaporizhya": 27,
							"Moved to west Ukraine": 27,
							"N/A": 0.1
						};
...
//		using(Box2D, "b2.+");
		...
		bule.each(function(d,i) {
			...
			d3.select(this).append("svg:use")
				...
				.attr('class', "over");
			d.statusOutline = d3.select(this).append("svg:circle")
					.attr('class', "status")
					.attr("r", radius-7).attr("stroke-width", 9)
//					.attr("stroke", 'red')
					.attr("fill", 'none');
...
//		$("#filters .button").on("click", changePeriod);
		$("#filters")
			.delegate(".button:not(.selected)", "click touchstart", changePeriod);
...
//						.attr("opacity", parseFloat(toFixed(d.value, 1)) ? 1:0);
				...
				if(d.value)
					d.statusOutline.transition().duration(600)
...
//			}, 500);
	...
	function setNodesType(type, dateStart) {
		...
		for (i = data.companies.length - 1; i >= 0; i--) {
			...
			node.radius = 2* statusRadius[node.status] * node.value;
			nodes[i] = node;
...
//						.attr("opacity", parseFloat(toFixed(d.value, 1)) ? 1:0);
				...
				d3.select(d.type_svg)
					.transition().duration(750)
						.attr("width", r/3).attr("height", r/3)
						.attr("x", -r/3/2).attr("y", -r/3/2);
				if(d3.select(this).select("use#show-selected-node")[0][0])
					selected.transition().duration(600)
						.attr("r", d.radius);
...
//		var cnt = 0;
	function animate() {
		...
		for (var b = world.GetBodyList(); b.ptr; b = b.GetNext()) {
