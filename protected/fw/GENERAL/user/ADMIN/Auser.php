...
class Auser
{
    ...
    /**
     * Deleteaza userul si il inlocuieste cu un user deja existent. Pentru ca sa
     * nu fie probleme cu recordurile si sa nu fie nici ele lasate in plop
     * defapt ar trebui sa se creeze un unknown author. sub care sa se poata posta
     * lucruri anonime.
     * @param $uid
     *
     * @uses
    ...
    function deleteUser($uid){
        $where = " WHERE uid = $uid";
        $queries = array(
            "UPDATE blogRecords SET uidRec = 1 WHERE uidRec = $uid",
            "DELETE FROM blogRecords_authors $where",
            "DELETE FROM auth_user_stats $where",
            "DELETE FROM auth_user_details $where",
            "DELETE FROM auth_users $where",
            "DELETE FROM auth_sessions $where"
        );
        $this->C->Db_queryBulk($queries);
        Toolbox::relocate(PUBLIC_URL."/?idT={$this->idTree}&idC={$this->idNode}");
        //var_dump($queries);
        return false;
    }
