...
class CsetModule extends CgenTools
{
    ...
    public function Module_Set_incFilesHard($extension,$srcPath)
   {
        ...
        if (file_exists($filePathYml)) {
            ...
            foreach($yml_array AS $var_name => $var_value)
            {
                $notEmptyArray = (  isset($mod->$var_name)
                                 ...
                                 && (!isset($mod->adminOverride) || !$mod->adminOverride)
                                );
    ...
    public function Module_Set_YamlProps(&$mod, $adminPrefix, $settings=''){
        $modType = $mod->modType;
        $modName = $mod->modName;
        ...
        $confRes = $this->Module_configYamlProps($mod, $filePathYml);
        /*echo ($confRes
                ? "<b style='color:green;'>TRUE $filePathYml </b><br>"
                : "<b>FALSE</b> $filePathYml <br>");*/
     ...
     */
    public function Module_Fs_configYamlProps(&$mod, $adminPrefix='', $settings='')
    {
        ...
        // daca nu avem template incercam sa vedem daca a fost cumva definit
        // in alta parte inainte
        //echo "settings [ {$modName} ] ".$this->settings[$modName]."<br>";
        if(!$settings) {
            $settings = isset($this->etcSett) && $this->etcSett[$modName]
                ? $this->etcSett[$modName]
                :( $mod->settings
                    ? $mod->settings
                    : '');

           // echo "$modName settings = $settings <br>";

            /**
             * Daca are sau nu template incearca sa apeleze default sau directorul cu template
             * daca il gaseste si templateul nu a fost predefinit
             * si atunci inseamna ca mod->template a fost setat din yaml
             * incearca sa vada daca nu cumva exista si niste setari adaugate setarilor default.
             *
             * deci pentru a avea setari directe pe template
             * - trebuie declarate in core / acore.yml pentru modulul respectiv
             * - sau modulul trebuie sa seteze cumva in __construct tempalteul lui
             *
             */
            if($this->Module_Set_YamlProps($mod, $adminPrefix,$settings)){
               if(!$settings && $mod->settings){
                    $this->Module_Set_YamlProps($mod, $adminPrefix,$mod->settings);
               }
            } elseif($settings || (isset($mod->settings) && $mod->settings)) {
                // daca totusi nu se gaseste nimic la templateul selectat
                // atunci incercam fara template
                 $this->Module_Set_YamlProps($mod, $adminPrefix,'');
            }
        } else {
            /**
             * Daca este apelata functia direct cu un template atunci => direct tempalate directory
             */
            $this->Module_Set_YamlProps($mod, $adminPrefix,$settings);
        }
