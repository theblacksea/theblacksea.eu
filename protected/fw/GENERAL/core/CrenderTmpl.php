...
class CrenderTmpl extends item
{
    ...
    public function Get_templateFromPath($tmplPath)
    {
        if ( file_exists(FW_PUB_PATH.$tmplPath)) {
           ...
           // echo file_get_contents(FW_PUB_PATH.$tmplPath);
            ...
            //echo "Nu exista template la calea ". FW_PUB_PATH.$tmplPath."<br>";
             return '';
        }
    }
    ...
    public function Render_assoc( &$aR, $obj='', $tmplType='str', $tmpl)
    {
        ...
        if (!$content) {
           error_log("Render_assoc: Templateul nu a putut"
        ...
        if (!is_object($obj)) {
            ...
            $content = $this->Get_template($tmpl,'Path', $tmplType);
             if (!$content) {
                 ...
                 if($this->fallBackTemplate && $tmpl!= $this->fallBackTemplate){
                     return $this->Render_object($obj,'path',$this->fallBackTemplate);
                 } else {
                 ...
                 //echo "$obj->modName Exista continut de template pt <b>$tmpl</b> <br>";
    ...
    */
    ...
    //special. variable sett
    static function set_value(&$prop, $var){
        $prop = $var;
    }
    static function set_ObjProp(&$obj, $name,$var){
        $obj->$name = $var;
    }
    public function setPreview(){
       $this->jsTalk = " fmw.preview = true;";
    }
