...
class Cmodules extends CsetModule
{
    ...
    static function Fs_deleteContentRes($dir, $prefix='')
    {
        //echo "in Fs_deleteContentRes<br>";
    ...
    public  function Set_module($modName, $modType){
        ...
        /*echo "*****[ivy] Cmodules - Set_module "
                    .(is_object($this->$modName)
                        ? "modulul {$modName} a fost instantiat <br>"
                        : "Modulul {$modName}  nu a fost instantiat <br>"
                    );*/
       /* if(!is_object($this->$modName)){
            echo "Modulul {$modName}  nu a fost instantiat <br>";
        }*/
    ...
    */
    ...
    protected  function Set_modules()
    {
        ...
        $mgrAction = $this->mgrAction;
        if ($mgrName) {
            //echo "Ccore - Set_modules : Current ModuleName is $this->mgrName <br>";
            //$this->Set_currentModule();
            $this->Set_module($mgrName, $this->mgrType);
            //if an action is set for the manager then call it
            // this can be coming from a request or from settings as a default action
            if($mgrAction && method_exists($this->$mgrName, $mgrAction)){
                $this->$mgrName->{$mgrAction}();
            }
        } else {
    ...
    protected function Set_currentManager($mgrName)
    {
        ...
        // deprecated "modName" should be used instead
            ...
            if(    in_array($this->mgrName,$this->GENERAL )) $this->mgrType = 'GENERAL';
    ...
    protected function Set_currentManagerAction($mgrAction){
