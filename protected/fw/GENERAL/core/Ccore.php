...
class Ccore extends Cunstable
{
    ...
    //===================[Project Setting]======================================
    public $tree;
    public $mtree; //a tree with all trees
    //public $tempTree;
    ...
    public $idTree;
    public $idNode;
    public $mgrName;    // set fromRequest, or from the tree
    public $mgrtype;
    public $mgrAction; // actioun request from route
    public $nodeName;
    public $nodeName_ro;
    public $nodeName_en;
    public $nodeResFile;
    ...
    //array cu setarile frameworkului / proiectului
    public $ivy_settings = array();
...
//===============================================================================
    ...
    protected function Set_currentPage()
    {
        ...
        */
        ...
        if(isset($_REQUEST['route'])){
            // handle routing ex: ?route=mgrName-mgrAction
            $routeParts = explode('-', $_REQUEST['route']);
            $this->Set_currentManager($routeParts[0]);
            $this->Set_currentManagerAction($routeParts[1]);
        }
    ...
    protected function Set_core()
    {
        ...
        /**
         *Daca primesc un request de schimbare de template si o cerere
         *de a-l face constant, atunci retin templateul
         *Daca nu inseamna ca este doar de data asta si il retin doar acum
         */
        $this->settings = '';
        if(isset($_REQUEST['ivySett'])){
            //$this->template = $_REQUEST['ivySett'];
            if(isset($_REQUEST['tmplConst'])){
                $_SESSION['ivySett'] = $_REQUEST['ivySett'];
            } else {
                $this->settings = $_REQUEST['ivySett'];
            }
        }
        /**
         * daca am setat un template si acel template nu este un string gol ,
         * atunci il retin
         * daca este un string gol, inseamna ca se doresc setariile default,
         * ale siteului si deletez variabila de sesiune.
         */
        if(isset($_SESSION['ivySett'])){
            if($_SESSION['ivySett'] && $_SESSION['ivySett'] != ''){
                $this->settings = $_SESSION['ivySett'];
            } else {
                unset($_SESSION['ivySett']);
            }
        }

        //echo "settings = ".$this->settings."<br>";
    ...
    public function get_settings(){
        ...
        while($row = $res->fetch_assoc()){
            $setting = $row['setting'];
            $value = $row['value'];
            $this->ivy_settings[$setting] = $value;
        }
    }
    ...
    function _init_($dbLink)
    {
        ...
        $this->mtree = $this->Get_masterTree();
