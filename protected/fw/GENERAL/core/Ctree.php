...
class Ctree extends Cmodules
{
    ...
    */
    function regenerateAllTrees()
    {
        ...
        $masterTree = $this->Build_masterTree(false);
        Toolbox::Fs_writeTo(FW_RES_TREE.'mtree.txt',serialize($masterTree));
...
/*        if(!$idTree) {
        ...
        foreach ($children AS $idCh) {
            ...
            $query = "SELECT name_ro, name_en, opt, type
                      FROM ITEMS
                      WHERE id='$idCh' ";
            ...
            //teoretic nu ar trebui sa fie "mod" din moment ce face referinta la
            // setarile unei pagini si nu ale unui modul
            $opt = array();
            if($qArr['opt']) {
                $options = json_decode($qArr['opt']);
                if(is_array($options)) {
                    $opt = $options;
                }
            }
    ...
    public function Get_masterTree(){
        $pathTree = FW_RES_TREE.'mtree.txt';
        ...
        if (is_file($pathTree)) {
            ...
            return $this->Build_masterTree(false);
        }
    }
