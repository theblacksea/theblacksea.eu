...
class Cprofile extends Cuser{
    ...
    {
        $row['editStatus'] = 'not';
        $row['photoSrc'] = $row['photo'] ? :  'RES/uploads/foto/default_avatar.png';
        return $row;
    }
    ...
    function _hookRow_aboutData($row)
    {
        ...
        $row['photo'] =  str_replace('http://theblacksea.eu/', BASE_URL,$row['photo']);
    ...
    //apeleaza metode in functie de template_file
    function _init_()
    {
    ...
    {
        // mtf = manager - template file
        if(isset($_GET['mtf'])){
            $this->template_file = $_GET['mtf'];
        } else {
            if($_GET['uid']) {
